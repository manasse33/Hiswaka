...
const DataDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Endpoint: GET /api/indicators/indicators/{id}/
        const indicatorRes = await api.get(`/indicators/indicators/${id}/`);
        setIndicator(indicatorRes.data);

        // 2. Récupérer les points de données (Valeurs du graphique)
        // Endpoint: GET /api/indicators/indicators/{id}/datapoints/
        // Note : Si l'indicateur est RESTRICTED et user non connecté, Django renverra 403 ici
        const pointsRes = await api.get(`/indicators/indicators/${id}/datapoints/`);
  ...
  const handleExport = async (format) => {
    try {
        // Endpoint: GET /api/datapoints/datapoints/export/?indicator_id={id}&format={json|csv}
        const response = await api.get(`/datapoints/datapoints/export/`, {
            params: { indicator_id: id, format: format },
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2 mt-2">
                        <span className="bg-emerald-100 text-emerald-800 text-xs px-2 py-1 rounded-full font-bold uppercase">
                            {indicator?.theme?.name || indicator?.category || "Général"}
                        </span>
                        <span className="text-gray-400 text-sm flex items-center gap-1">
                            <Database size={12}/> Source: {indicator?.source?.name || indicator?.source || "N/A"}
                        </span>
                    </div>
                </div>
